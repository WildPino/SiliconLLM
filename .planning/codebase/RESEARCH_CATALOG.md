...
| Does a 10B carved-int8 artifact exist and compute correctly? | **Yes:** E63 Part A, exact arithmetic/parity. Part B is hash-pinned and CONFIG-gated; three further 2026-09-16 preflights around and after user process cleanup refused before timing (42/45, 11/45, 22/45 occupancy breaches). Clean rate is still owed. No further process cleanup should be requested from the user. | canonical E63 §15 / `e63_part_b_preflight.json` |
...
- Do not count E63's 2026-09-16 preflight refusals as timing attempts: no cell ran and canonical `e63_part_b.json` was absent. The runner's pinned inputs remain unchanged; reuse it only after its strict guard passes. Do not ask the user to close ChatGPT or repeat Part A/parity/interim as substitutes. A loaded-box protocol would be a separately registered result, not `G-E63d`.
...
| E63 interim/addenda | paired int8/packed ratio in contended run ≈0.9826 corrected; composed estimate ≈49 tok/s, CI ~45.5–51.1; no absolute claim. Addendum D pins runner/engine/artefacts/sidecars and gates every future `CONFIG`. Three further 2026-09-16 preflights around and after process cleanup refused at 42/45, 11/45 and 22/45 breaches, before any timing cell; `e63_part_b.json` remains absent and `G-E63d` is unattempted/VOID/OWED. | monitor for a spontaneously quiet window without asking user to close ChatGPT; any loaded-box successor must be preregistered separately |
...
3. **Do not demand more user process cleanup for `G-E63d`:** its hourly 45 s all-samples guard may eventually pass, but three new windows failed after cleanup. No timing cell ran. If the host cannot satisfy the frozen gate, preregister a distinct loaded-box study and never relabel it `G-E63d`.
