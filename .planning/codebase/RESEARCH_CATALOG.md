...
| What did H4 aggressive-rank Stage A establish? | **Training signal, but generation still at floor.** Stage A on one T4 is `COMPLETE`; terminal `TIME_CAP` checkpoint at step 1148/1145 applied is hash-verified and CPU fp32 evaluated. BPB init 2.8035765 → terminal **1.1537375**; teacher-forced 21 → **66/160**, free 5 → **6/160**, mean rank 3094.775 → **108.725**, rank≤5 39 → **112/160**. `TRAINING_HELPS`, `BEATS_QO96`, `BEATS_QO192` true; `GENERATOR_PARTIAL` false. No combined PASS, 10B/rate/export claim or automatic Stage B. | canonical H4 §10 / terminal CPU JSON |
| Is H4's saved fp32 latent checkpoint secretly better than its ternary forward? | **No on the same masters:** preregistered H4D removes only A/B ternarization and worsens BPB **1.1537 → 1.6104**, teacher-forced **66 → 54/160**, free **6 → 1/160**. This is a post-hoc diagnostic, not H4 re-adjudication; it does not test separate fp32 retraining or prove a rank ceiling. | H4 §11 / H4D brief and JSON |
...
- Do not repeat H4 step zero or Stage A: its terminal `TIME_CAP` checkpoint was adjudicated only by the frozen CPU fp32 evaluator. Do not select an interim checkpoint, substitute E65 QO-48 for the H4 init anchor, or launch Stage B without a new brief and separate GPU decision.
- Do not propose "turn off quantization" on the H4 terminal masters as an untested rescue: H4D already measured that exact ablation and it worsened all quality metrics. Different *training* in fp32 is a different, still-unmeasured treatment.
...
| H4 step zero / Stage A | **TERMINAL CPU EVALUATED:** rank48 ternary q/o on pinned 1.5B donor, Stage A `TIME_CAP` step 1148. Terminal 1.1537375 BPB, 6/160 free, 66/160 teacher-forced; `TRAINING_HELPS`, `BEATS_QO96`, `BEATS_QO192` true, `GENERATOR_PARTIAL` false. P1 numeric and P3 missed; P2/P4 hit. | strong score/rank recovery but free generation remains AT-FLOOR; no Stage B without new brief; no 10B/rate/transfer/export claim |
| H4D latent masters | **DESCRIPTIVE / COMPLETE:** same terminal trained checkpoint, bypass A/B ternarization only; fp32 masters 1.6104023 BPB, 1/160 free, 54/160 teacher-forced versus ternary 1.1537375, 6, 66. Two predicted improvements MISSED; floor prediction HIT. | no hidden fp32 rescue in the saved STE masters; no inference about separately trained fp32 or rank capacity; no H4 gate promotion |
