...
**arm3 is retained as a record-only cross-check, not the anchor:** does CE-on-whole-corpus (arm3, 1.1377) match CE-on-logit-window (KD-α0)? If yes, restricting to the window introduces no domain bias and the α trend is read on a representative domain; if no, the window itself is a variable and that fact must be known before the trend is believed — a free positive control on the domain restriction.
