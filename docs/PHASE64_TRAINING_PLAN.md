...
## 11. MVE apparatus returns (2026-07-13) — adjudicated

- **KD implementation switched to chain-rule span factorization (`--kd span`), sealed before the MVE run** (pushed in the same pre-launch session, immediately after the prereg — nothing executed) — full rationale in `PHASE64_DECISIONS.md` D3: the anchor projection retains 13-15% of teacher entropy (many-to-one collapse; smoke CE 2.081 < KD-anchor 2.729); the chain-rule form retains 83-86% at zero extra teacher cost. The D3 gate itself (span-KD must beat CE at the pilot) is unchanged; `MVE_PREREG.md` records the switch, the retention table, and the smoke numbers as the pre-run rationale.
- **Teacher economics re-metered (supersedes §2's desk-brackets): offline KD is teacher-forced *scoring* (prefill only), not generation.** Measured: **2283 tok/s on the 3060** (fp16, ctx 2048, stride 1536, b4; batch is not the lever — the 151K-vocab softmax was, bypassed via top-K on logits + row logsumexp). At this rate the 1-2B-token KD subset ≈ 5-10 days on the 3060 *alone*; T4/5090 shift that to days/hours. **The "generation costs more than training" crunch largely dissolves**; STOP-B re-prices on this number plus the MVE-measured train throughput. Logits stored **per teacher-token** (not per anchor) — keeps V open for the rung-1 A/B and is cheaper; it is also what the span method consumes.
- **Upcycle magnitude-matching bug found and fixed (banked as a law for stages E at every rung):** dense down-proj sums all hidden units; the seeded MoE weights each expert by router weights summing to 1 → the seeded model emitted ~1/k of the dense output → +0.49 BPB transition shock, +0.0056 after magnitude-matched seeding. Any upcycle at any rung must verify transition-BPB continuity.
