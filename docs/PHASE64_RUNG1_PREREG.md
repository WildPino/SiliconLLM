...
**Budget unit — declared, because the vocab arms make it load-bearing (added 2026-07-19).** Each arm's 15% is counted in **student tokens**, i.e. equal optimizer steps and equal compute. Measured consequence: at 2.562 B/tok (V2048) versus 2.850 B/tok (V4096), **the V=4096 arm sees ~11% more bytes for the same step budget**. That is not a nuisance to be corrected — it is part of what a larger vocabulary buys, and the byte-normalized metric makes the arms comparable regardless. But it must be **stated in the claim**: the screening answers *"at equal token budget, which vocab yields lower code-val BPB"*, and any quotation of the result carries that qualifier. The alternative unit (equal bytes) answers a different question and is not what the ladder's token-denominated budget matches.
