...
> | v5 — α declared | this commit | §4.3 the span-KD challenger no longer inherits an unnamed α: it runs at **α ∈ {0.5, 0.25}** and is **read as a trend, not a best-of**, so closing D3 permanently now requires a coherent monotone pattern where a single point previously sufficed. Raised by an external commenter (relayed by the MM) in general form; the specific mechanism — a directional gradient blend over ~97% of positions under span mode, not a magnitude effect — is ours. **Tightens**; costs one arm (§9: 33% → 41% of a rung) |
> | v6 — measured-constants refresh | this commit | Two figures updated after the ex-band BPE retrain re-measured the teacher/student ratios at 0.637 / 0.709: §4's byte-advantage template **+11.2% → +11.3%** (B/tok now ratio-derived, 2.564 / 2.854), and §4.1's tie-breaker rationale **"17% shorter student sequence" → ~10%** — the 17% did not reproduce from any measured pair and is corrected before launch, not defended. **No gate, threshold, or tie-breaker direction moves**; the trial-slice span statistics in §4.3 are noted as pre-retrain and re-derived from production logs |
>
...
Tie-breaker: **V=4096** (records a **~10% shorter student sequence** — 1 − 0.637/0.709 = 10.2% from the re-measured ratios; the "17%" quoted before v6 did not reproduce from any measured pair — compute saved in training and bytes/s at inference).
...
Arm inventory: **screening 5 arms × 15% of stage C ≈ 41% of a rung** + **main run 100%** + **three E+F branches ≈ 75%** ≈ **2.1 rung-equivalents**, i.e. roughly **+108% instrumentation overhead** against the +50-80% assumed in `PHASE64_TRAINING_PLAN.md` §5.
