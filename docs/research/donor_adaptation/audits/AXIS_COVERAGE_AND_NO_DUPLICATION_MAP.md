...
| **Routing and selection** | learned gate predictors; static/oracle routers; trained routers; granular MoE | F1 characterizes predictability; E23/E37 price real router and show it is load-bearing; H1 shows joint training helps and router still moves; H2I is the one-byte continuation | post-hoc **CLOSED**; trained one-byte route **ACTIVE** | Do not treat teacher-forced router behavior as free-running success. Do not compress/ternarize an H2I fp32 router inside the identity arm; that is a separate treatment |
...
No newly discovered local result licenses an extra GPU launch today. H1/H2I already occupy the two
high-information training cells; the remaining immediate work is event-driven adjudication and the
quiet local `G-E63d` measurement.
