...
| **Sparsity, pruning and carve granularity** | magnitude/block pruning; coactivation/null; neuron/expert granularity; static and mass-ranked carve; 1.17% FFN; coarse locality | D1/D0/D0c/E19/E27/E33/E37/E38 measure post-hoc sparsity at the target active budget; E38's top-`k` **intermediate-mass** selector is above chance at `k=3`, but is not an output- or loss-optimal oracle. E64 shows carve is dearer on int8 | mass-ranked post-hoc route **CLOSED**; output-aware selection and shared-path residual **UNMEASURED** | Do not repeat E38's mass selector or claim it bounds all selectors. A distinct output-aware criterion needs a planted control, exact byte parity, held-out FFN-output error, then end-to-end BPB/rank before deployment promotion; see E67 brief |
| **Routing and selection** | learned gate predictors; static/mass-ranked selectors; trained routers; granular MoE; frozen H4+H2I cross-composition | F1 characterizes predictability; E23/E37 price real router; E38 rejects mass-based selection at `k=3`, not every possible criterion; H1 S3 closes ternary 8-layer joint trainability; H2I v4 learns held-out score and beats STATIC routing but fails strict combined rank; H5 frozen composition is adverse | mass-based post-hoc **CLOSED**; output-aware post-hoc **OPEN/UNMEASURED**; ternary 8-layer trainability **MEASURED**; one-byte 8-layer score+rank **CLOSED `SCORE-ONLY`**; frozen H4+H2I transfer **CLOSED negative** | Do not treat teacher-forced behavior as free-running success, or local output-error gains as BPB/rate. H5 does not disprove fresh joint training; H2I's BPB/router win is not an export, rank, rate, scale or donor license |
