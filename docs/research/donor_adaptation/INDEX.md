...
**Last updated: 2026-09-08 (**E16 `SCORE-CROSSES-RANK-DOES-NOT`: the rule was MOST of what was broken, and fixing it STILL does not produce a working model** (`probes/E16_R3_AT_7B.md`). Same slice, same protocol, same binary as E15, one axis moved at a time. **Swapping the shipped rule `R0` -> `R3` with the fold and everything else held FIXED is worth -1.281967 BPB at 7 B** and carries the model from +1.229094 ABOVE the chance line to **4.017233, -0.052874 BELOW it** -- 59x the band, so `G-R1` reads its registered label **`RULE-FIXES-IT`**. **And it agrees with its own fp32 donor on 0 of 160 greedy tokens, diverging at token 0** -- identical to E7's planted control, the artifact E15 measured 1.229 BPB WORSE than guessing. **B2 is 0.155 nats/token better than uniform and cannot rank the donor's next token even once.** **`RULE-FIXES-IT` is true of the band and false of the model**, and **E16's brief registered ONLY scoring metrics** -- G-R1 through G-R4 are all BPB. **E14's law, written one day earlier against a different experiment (pair every SCORING metric with a RANKING one), is what caught it**; the ranking run was added afterwards and **REPORTS rather than decides** (E14 s6), so the gap in the registered instrument set IS part of the result. It validates itself first: B1 reproduces E7's published `0/160` against E7's own stored fp32 continuations. **The planted control FIRES hard**: C0 (the 1.5 B R3 artifact) reproduces E1's 3.475706691632780 to **3.20e-07** against a 0.01 tolerance, so the instrument is verified as STILL firing in the configuration that produced these nulls. **R3's damage grows with scale too, and it is NON-MONOTONE**: fold-none reads +0.461 above the line at 0.5 B, **-0.594 below at 1.5 B**, and **+0.098 ABOVE again at 7 B** -- E12 found the same of `R0`; R3 merely starts better and takes one more octave to show it. **The 1.5 B -> 7 B step costs +0.692619** but crosses model families, so it is NOT reported as a pure scale term. **The FOLD is load-bearing and is the one thing the shipped artifact had RIGHT**: `G-R4 = -0.151093` at 7 B, and it is the ENTIRE reason B2 sits below the line where B3 (fold none) sits above. Its credit shrinks consistently with scale -- **-0.529 (0.5 B, E2) / -0.220 (1.5 B, T3) / -0.151 (7 B)** -- which is exactly by how much E15's amended prediction was wrong to import the 0.5 B figure. **The design gate passed before any BPB was read**: B2 differs from B1 only on `rule`/`calib_seqs`/hash, B3 from B2 only on `fold`/`n_gains_folded` (R3 zeroes **54.84%** of weights against R0's 35.72%; no speed number moves, the packed format stores every code regardless). Predictions scored: **G-R0 INSIDE**; **G-R1 direction RIGHT, band missed by 0.018**; G-R2 is **the same single constraint as G-R1**, one miss not two; **G-R3 DIRECTION WRONG**; export predicted 1-3 h, measured 0.51 h and 0.46 h. **G-R3's reasoning failed before its band did** -- a trend extrapolated from TWO points in the same configuration, which the brief defended as legitimate against E15's cross-configuration composition and in the same paragraph admitted could not establish curvature. **REGOLA NUOVA: un trend letto da due punti e una DIREZIONE, non una legge.** **Where it leaves the goal: 6.79 tok/s exact stands and s19.3 is unchanged** -- the engine is at 97% of this machine's demonstrated streaming rate with the kernel on its own ceiling at every footprint, the donor is excellent (0.674027 fp32, monotone in scale), **the conversion destroys it, the rule was most of the destruction, the fold is load-bearing, and what remains after fixing BOTH is still at chance.** E15's conditional did not fire as written (B2 landed BELOW, not above) but **the greedy result reaches the same place by another route: with the honest qualifiers that only two of four rules were tested and no clean scale axis was available, the binding constraint at this scale is the FORMAT, not the rule** -- and *training INTO the format rather than converting into it*, `SCALEUP_ARCHITECTURE`'s premise, becomes a measured conclusion rather than a preference. Previously: **E15 `DOES-NOT-PREDICT`: the artifact that EVERY donor tok/s in this programme is quoted on scores WORSE THAN GUESSING** (`probes/E15_DOES_THE_7B_PREDICT.md`). Full 24x512 heldout slice, `--seqlen 512`, chance line **4.070106**, band **0.000896**: **`qwen25-coder7b_p.bin` reads 5.299200075 = +1.229094 ABOVE chance, which is 1,372x the band** -- no vocabulary convention, slice choice or protocol detail comes within three orders of magnitude of closing it. **The planted control FIRES and fires hard**: the same weights in fp32 read **0.674026555, -3.396080 BELOW** the line. **So the engine is not the problem and the donor is not the problem -- the CONVERSION is.** The 7 B is in fact **the best model this programme has ever run**, and the fp32 sweep on the identical slice is monotone in scale (0.871795 / 0.767595 / 0.724450 / **0.674027**) -- a code-specialised donor winning on a prose-majority corpus (40% pg19 / 25% markdown / 25% python / 10% wikitext), which is also the first evidence bearing on the open "pull a real ~10 B" question: **bigger donors keep paying, in fp32.** **E7 said so first and E15 only put a number on it**: section 5 wrote "R0 at 7 B collapses completely, and how much of that is the rule versus the scale is a separate experiment nobody has run", and deliberately reported no BPB -- **what happened in between is that 4.46 and then 6.79 tok/s went on being quoted as the rate on the real donor for a month while that sentence stayed true and unmeasured.** **E7's two headline halves are about DIFFERENT artifacts**: the `160/160` is the fp32 arm, the packed arm is E7's own planted control at `0/160`, diverging at token 0 on 5/5 prompts. **NO SPEED NUMBER MOVES, and the reason is structural -- a ternary weight costs the same bandwidth whatever scale multiplies it**, so 6.79 tok/s exact, 24.0 GB/s, 48.0 G-weights/s, E8's 1.349x, E9's 1.056x, E10's `CORE-BOUND`, E11's `NO-LIFT`, E13's 1.358x lever and E14's `CHEAP-BUT-NOT-NEUTRAL` all stand. **What E15 removes is the right to call 6.79 tok/s a rate for a WORKING 7 B.** For the format's speed value there is a ratio and not a rate: E7's generate stage ran both arms in one run at fp32 1.382-1.392 and packed 4.853-4.888 tok/s = **3.51x**, **generate-stage figures and NOT witnessed `--bench` rates** (section 11's contention witness postdates them), so the ratio is the durable part. **This programme has a 7 B that predicts and a 7 B that is 3.51x faster, and they are not the same 7 B.** Predictions scored honestly: the control's **0.55-0.85 CONTAINED** 0.674027, while the amended **3.4-5.2 MISSED** 5.2992 that the withdrawn 4.6-6.5 would have caught -- the amendment composed E2's fold credit (0.5 B, R3), T2's rule penalty (1.5 B, FFN-only, fold none) and a scale term across **three different configurations**, **the exact move E12 was written to forbid, which the brief named as forbidden before the run and made anyway**. **Rule: composing measured deltas across configurations is not conservative just because each delta is measured** -- and refusing to call the direction cost most, since E12's R0 sweep and E7's 0/160 both supported the call that was declined. **OWED AND NOW LOAD-BEARING: B2 -- re-export the 7 B with `--rule R3 --calib-seqs 32`, keeping `--fold layers` and `--head-ternary` so the RULE is the only variable**, and re-measure. This is E7 section 12 owed item 3, promoted from optional. **R3 is the rule the pipeline actually SHIPS** (`e1_bpb_through_engine.py:395`) **and it works at 1.5 B: E1 reads `TQH` at 3.475707, 0.594 BELOW the line** -- a ternary 7 B that predicts is not ruled out by anything measured, it has simply never been built. **If B2 also lands above the line, no rule in this exporter produces a working 7 B, the binding constraint moves from the rule to the FORMAT, and `SCALEUP_ARCHITECTURE`'s premise -- train INTO the format rather than convert into it -- stops being a preference and becomes the measured conclusion.** **E16 has since CLOSED -- see the head of this entry.** Previously: **E14 `CHEAP-BUT-NOT-NEUTRAL`: the 1.358× lever's quality cost is MEASURED at last, and the pre-registered band cannot see what it measured** (`probes/E14_ACTIVATION_INT8_COST.md`). At the 1.5 B verdict cell, **0.625503 BELOW the chance line** where BPB means what it usually means, int8 activations **LOWER** bits-per-byte: `dBPB(A1) = -0.016961`, `dBPB(A2) = -0.005991`. §4's bands are **one-sided**, drawn for a cost, so a negative delta falls into `ACTIVATION-CHEAP` **by construction** — and that is what the runner printed. **G-N3 says it is not free: whole-vector int8 reproduces the fp32-activation greedy trajectory on 45.6% of tokens** (diverging at token 0), group-32 on **64.4%** (token 1). **The top-1 token changes on 54% of positions while the score improves** — BPB scores, greedy ranks, and noise that softens an over-confident logit vector improves the first without improving the second. **`ACTIVATION-CHEAP` is true of the band and false of the model, and the registered instrument set had no gate that could tell the difference** (brief §6 forbade promoting G-N3 after the fact, correctly — so the gap is the result, not something patched over). **G-N3 was itself pre-registered and never implemented**: `e14_activation_cost.py` runs `--bpb` only; it was written and run afterwards against A0 exactly as registered. **The lever is NOT released, and for a second reason: the composition is CROSSED.** E13 measured 1.358× on `--lutblk` alone, which G-N0 and G-N3 both show is arm **A3 = A1** — the arm with the LARGER quality move — while the quality-cheap arm **A2 has no speed number at all**; the flags are orthogonal so `--lutblk --lut-group 32` is reachable, but **Phase 61's law forbids carrying 1.358× across to it**. **So 6.79 tok/s exact stands, packed stays DEFAULT, and the lever stays unspent — now for a measured reason.** Confirmed in passing: **E13's bit-identity holds THREE independent ways** (sha256 over 311 MB of logits, `A3-A1 = +0.000e+00` at both cells, and token-for-token identical greedy over 160 tokens). **G-N1 fails at BOTH cells in raw form and fires at both repaired**, `|A2-A0| < |A1-A0|`, separating 24× and 2.83× with the more accurate arm nearer the reference — the harness was never at fault, the direction written into the gate was. **Two of three gates assumed the answer's shape**: G-N1 a DIRECTION that holds only while the model predicts, G-N2 a SIGN that holds only while quantization damages — **fifth pre-registered rule in this programme aimed at the wrong number, and the second inside one experiment**. **Protocol defect recorded, not smoothed: E14 passed no `--seqlen` and `donor_engine.c:1249` defaults `SL = n`, so every arm was scored as ONE 12,288-token sequence, not 24 documents of 512** — which is why A0 reads 4.629292 at 0.5 B where E1 reads 4.531234 on the identical file; the arm-to-arm gates are unaffected and a `--seqlen 512` re-run is owed. **Also filed today: a correction to E7** — its brief says both arms are `--fold none`, but the packed sidecar reads `"fold": "layers", "n_gains_folded": 56`; no speed, parity or greedy gate moves. **And E15 was pre-registered the same day and has since CLOSED — see the head of this entry.** Previously: **E12 `CHANCE-LINE`: the rule the exporter SHIPS puts every donor past chance, at every scale tested — so the pre-registered question could not be asked, and the first write-up of E12 (`CONTROL-FAILED`) is WITHDRAWN** (`probes/E12_TERNARY_COST_VS_SCALE.md`). Every ternary quality number this programme has ever used was measured at ONE shape (`T2_TERNARIZATION_RULE.md:4` = Qwen2.5-1.5B), and with E10/E13 closing the engine side that assumption became load-bearing for the whole remaining 5.4x. E12 drew the curve at 0.5/1.5/3 B: dBPB **3.715658 / 4.738239 / 5.010249**, each donor against its OWN fp32 baseline, rule IMPORTED from `t1_ternarize`, which is what `qwen_export.py` calls. **CORREZIONE dopo il push: `R0` e il DEFAULT del flag ma NON la regola che il programma SPEDISCE** — ogni artefatto motore standing e esportato con `--rule R3` (`e1_bpb_through_engine.py:395`), quindi **E12 ha misurato una regola che la pipeline non usa**; e E1 ha gia due terzi dello sweep giusto, sulla stessa slice: **R3 legge 4.531234 a 0.5 B (+0.46 SOPRA il caso) e 3.475707 a 1.5 B (−0.59 SOTTO) — la regola spedita ATTRAVERSA la linea fra 0.5 e 1.5 B e il punto di attraversamento e IGNOTO**; manca la cella 3 B, esattamente dove `R0` si e invertita. **The constant that decides it costs one line and I never took it: `log2(151936)/4.229452` = **4.069819 BPB** is what a model scores guessing UNIFORMLY, and EVERY ternarized arm at EVERY cell is ABOVE it** (FA +0.518 / +1.436 / +1.665 past chance) while every fp32 baseline is far below (0.72-0.87). **Above that line BPB measures how confidently WRONG a model is, not how damaged it is** — so `r = 1.3484` is a ratio of distances in a regime where distance has stopped meaning cost, and it is NOT reported. The deterministic `F` > `FA` inversion at 3 B (5.928395 vs 5.734699, **more organs converted, LESS damage**) needs no instrument bug: it lives entirely above the line. **The instrument was never the problem** — `I - base = +0.000e+00` exactly at all three cells over 168/196/252 tensors, counts structurally correct, tokenizers verified identical, baselines monotone. **What WAS the problem is that I re-adopted a control this programme had already retired**: T2 §4(a) measured `R0 - Z = -0.064 +/- 0.126` and wrote that arm Z "was the wrong control", the same amendment is in the docstring of `t1_ternarize.ternarize` — the function E12 IMPORTS — and t1's own `ARMS` table labels the row mis-specified. The sanctioned control is arm `I`, **and it passed everywhere**. Caught by E12's `Z` failing to reproduce T2's `Z` at the SHARED cell while `F` reproduced to six decimals (+3.309099 both): an **off-by-one in which per-tensor seed lands on which tensor** moved `Z` by **0.628 BPB = 126 sigma_seed**. Measured directly (`ternary/e12_zvar.py`, pre-registered `ae403c2`, 5 draws/cell): **spread(3B) = 1.695938 -> `Z-UNSTABLE`, predicted and landed — THIRD band in this programme to contain its own result** (after E9, E13). **The 3 B spread is LARGER than the whole 1.490 gap the withdrawn verdict was read off**, and at 0.5 B the control's outcome FLIPS with the seed (2/5 draws put Z below F, where the first write-up recorded "fires, +0.019"); it fires 5/5 only at 1.5 B, which is why T2's shape looked healthy. **It also re-reads T2: `R0` — the shipped default — sits +0.007 ABOVE the chance line and `Z` +0.070, so §4(a)'s contrast was between two points both PINNED AT CHANCE and could not resolve; the signs are `sign(w)` under every rule in that table and R3/R5 carry the identical signs 1.6-2.0 BPB BELOW it.** T2's `RULE-HELPS` stands; its §4(a) MECHANISM is withdrawn, and what its 62% bought was **the difference between a model at chance and a model that predicts**. **Owed first: FINISH the R3 sweep — the 3 B cell.** E1 already has the shipped rule at 0.5 B and 1.5 B and it CROSSES the chance line between them; the missing cell is 3 B, exactly where `R0` inverted. **Until it runs this programme has NO measurement of how the SHIPPED conversion scales, only of how `R0` fails.** New rules: **a control must be checked as still SANCTIONED, not merely labelled**; **a stochastic comparator needs its own dispersion before one reading of it decides anything**; **a control that passes on a SMOKE is not thereby a control** (+0.147 smoke vs +0.019 full); **read a converted model's BPB against `log2(V)/bpt` BEFORE concluding anything**. Previously: **E13 `LAYOUT-CONFIRMED`: the LUT kernel's collapse was the LAYOUT, and fixing it is worth 1.358× at donor scale — on a LOSSY path** (`probes/E13_BLOCKED_TILE_MAJOR.md`). E10 had left one row of its own table unread: at 2048 MB its fp32 arm streamed **38.58 GB/s** while its packed arm moved **26.48** — same instrument, same threads, same *moved*-byte convention — so `CORE-BOUND` left **1.46× measured**, not zero. E11 blamed the LUT collapse on layout (`t*Mpad` puts consecutive reads 299 KB apart) and **never tested it**; E13 changes nothing else. `build_tm` stored `tm[t*Mpad + o]`, so consecutive 32-byte reads sat **18.5 KB** apart at `gate|up` and **3.5 KB** at `down` — a different page nearly every read, 32 bytes used of every 64-byte line, for the **same total bytes**. `--lutblk` stores each 32-row tile contiguously: same bytes permuted, `t` order and accumulate tree untouched, so **bit-identical — the gate is sha256** (`--logits` 512 tokens, 311 MB, `7a9b3e04…f48e44` on both arms). **G-M1 = 67.51 G-w/s at the 512 MB verdict cell E11 named in advance, against a ≥65 boundary, and the brief predicted 65–77** — second prediction in this programme to land inside its own band, derived like E9's from a *measured* quantity over a *structural* factor. **Blocking recovers 3.0–3.5× at the two largest cells; the 4.2× L3 cliff becomes 1.53×; and 103.05 G-w/s at 12 MB is the fastest weight kernel this programme has measured.** At 512 MB `lutblk` moves 33.76 GB/s against the fp32 arm's 34.75 in the same sweep — **97%**, so **§25.1's 1.46× is taken and there is no third kernel to write**. **It composes, with the right cell**: Coder-7B's fused organs are `gate|up` 67.9 MB and `down` 33.9 MB, the 24–48 MB band where the sweep gives 1.83 and 1.51; Amdahl predicts 1.3–1.4× and the engine measures **1.358×** (9.21 vs 6.78 tok/s, 5 interleaved reps) — **the 512 MB cell's 1.19× was never the engine's number**. `--lut` as shipped reads **0.557×**, which **confirms E11's `NO-LIFT` end-to-end** rather than softening it, and E11's §12/§2.1 corrections stand; what is overturned is only its untested §3, which now **holds**. The known-positive reproduced to **0.15%** (packed 6.78 vs E9's 6.79, `ffn~` 115.8/117.3/117.4 against the published 114.5–117.0 plateau). **But this is NOT a like-for-like 9.21**: the LUT path quantizes activations to int8 at a measured **1.40e-01** relative L2, **never carried to BPB or greedy parity on the donor**, so **the honest speed is still 6.79 exact with a 1.358× lever whose quality cost is unmeasured** — and the packed path stays the default. Recorded against itself: a first 3-rep pass at 0.5 B gave `lutblk` a **180% spread** and is discarded as warm-up — **three reps were not enough at that shape**. **§19.3 is better supported than this morning: the remaining 5.4× is a property of the model, and E12 is the experiment that decides it.** Previously: **E11 `NO-LIFT`: the `--lut` kernel is refuted as a speed lever — and the two kernels SWAP PLACES at exactly the 16 MB L3 boundary** (`probes/E11_LUT_KERNEL_CEILING.md`). At the donor's real footprint the LUT path is **2.5× SLOWER per weight** than packed (21.90 vs 55.95 G-w/s at 512 MB, against a ≤1.15 NO-LIFT boundary fixed before the arm was written). **This corrects §12 and INDEX §2.1**: the published «`--lut` = 1.05×» and «`--fuse --lut` adds 3.9%» were measured against a packed arm that still had E8's single accumulator, while `matvec_lut` already had four — post-E8 the ordering is reversed. **But inside L3 the LUT kernel is the FASTEST weight kernel this programme has ever measured, 91 G-weights/s, 1.8× packed** — and it falls **4.2×** across 16 MB where packed (E10) does not move at all. The mechanism is the LAYOUT, read off the code: `codes + t*Mpad + base` puts consecutive reads **299 KB apart** at donor scale, so **tile-major is what makes one `vpshufb` serve 32 rows and the same thing that destroys the stream**. That is exploratory — not in the pre-registered bands — but it is 1.8×, reproduced twice, and it lands on `SCALEUP_ARCHITECTURE`'s cache-resident keystone ≤16 MB with experts streamed, which is probe-3's boundary exactly. Two instrument failures recorded rather than smoothed: **run 1 was VOID** (a concurrent worker in another checkout streamed the 30.46 GB fp32 donor across six threads; the packed arm acquired a slope it does not have, and **only a number carried in from a previous session could catch it** — every cell was contaminated together, exactly as with E8's discard rule), and **G-L2 then failed by MIS-SPECIFICATION** — I drew its band from one of E10's two sweeps and ignored the other, the **fourth** pre-registered rule in this programme aimed at the wrong number. **The goal is unmoved: §23.3 stands, packed remains the weight path with ~1.03× of headroom, Coder-7B is 7.4× short of 50 tok/s.** Previously: **E10 `CORE-BOUND`: the packed kernel is NOT bandwidth-bound, and §19's load-bearing assumption is now MEASURED and FALSE** (`probes/E10_PACKED_KERNEL_BINDER.md`). The same `matvec` source, on a synthetic matrix swept from 4 MB to 2 GB with the row length held fixed, delivers **49–53 G-weights/s at EVERY footprint** — a **512× change in working set moves it under 8%**, while the fp32 arm in the same run moves **4.4×**. **The engine is already on that ceiling**: `gate+up` 51.66 and `down` 52.22 G-w/s against a kernel that cannot exceed ~53 anywhere. The **planted control fired first** — fp32 L3÷DRAM 4.40 against a ≥2.0 gate — and reproduced probe-3's 16 MB L3 cliff unprompted (161.7 → 104.5 → 45.1 GB/s), and G-K2 tied the bench to the engine's own 26.1 GB/s within 2.3%. **So §19.4's 37.0 and 42 GB/s budget rows are WITHDRAWN for the packed path** — §19.5 had named that assumption as its own weakest link. The weight path's share of the remaining engine budget falls from ~1.15–1.65× to **~1.03×**: more requires REPLACING the kernel, not feeding it. Two things it returned unasked: cache-resident, packed is only **1.27×** faster per weight than fp32 while reading **8× fewer bytes** (the format's byte advantage is spent once bytes stop being the constraint — out of cache it is 5.6×); and the loop issues **0.29 FMA/cycle/core out of 2**, so CORE-BOUND names the side, not the mechanism — **E10 offers no mechanism for the remaining 7× and says so.** Honest against itself: **G-K3 was aimed at the wrong statistic** (the 4 MB cell's own 30.6% spread is wider than the gap to the verdict boundary, so that ratio cannot decide) — third pre-registered rule in this programme pointed at the wrong number. **§19.3's conclusion is STRONGER: the missing 7.4× now has less engine cover than it had this morning.** Previously: **E9 `GLUE-CONFIRMED`: the SwiGLU glue ran on ONE thread** (`probes/E9_GLUE_PARALLEL.md`). E8's FFN decomposition charged, for the first time, a component that moves no weights at all: `silu(g[i])*u[i]` over 530,432 elements per token, **11.346 ms = 7.5% of the whole token**, with no `#pragma omp parallel for` while every `matvec` around it used six threads. Because it is an elementwise map with no reduction, **the gate is sha256, not parity** — `--logits` output byte-identical before and after. **6.45 → 6.79 tok/s, paired median 1.0557 over ten pairs**, glue 11.346 → **2.938 ms** (3.86×), profiled 6.59 → 7.12. **Predicted 1.055–1.066 in the pushed brief and landed inside it** — the first band in three to contain its own result, and the first derived by E8 §3's rule (a *measured* quantity over a *structural* factor) instead of from first principles. `rmsnorm` was left serial **on purpose and said so beforehand**: 0.330 ms over 56 calls against 2.7 µs per OpenMP region — a remedy has to clear its own overhead. Today's two changes total **1.52×** (4.460 → 6.79 tok/s, 24.0 GB/s, 48.0 G-weights/s); ledger §19.3's 1.7–2.5× engine budget is now **1.15–1.65×** remaining, and the **7.4× still missing is a property of the model, not the code**. Previously: **E8 `CHAIN-CONFIRMED`: the weight path was NOT bandwidth-bound, it was a DEPENDENCY CHAIN** (`probes/E8_MATVEC_DEPENDENCY_CHAIN.md`). Every `matvec` accumulated into ONE register, so the loop ran at Zen2's 5-cycle FMA **latency**, not its 2/cycle throughput — read in the emitted assembly before it was claimed, and surviving only because `-ffast-math` is forbidden here. Four independent accumulators: **Coder-7B 4.73 → 6.37 tok/s, paired median 1.3491 over ten pairs with a 1.8% spread**, weight path 16.7 → 22.5 GB/s, and **all four weight organs moved 1.41–1.49× from one change to one loop**. Parity-gated, never bit-exact: 3.3e-06 from the single-chain arm against its own 1.5e-05 to PyTorch, and 160/160 greedy identical. **The negative control held** — the same flag on the fp32 kernel, predicted 0.99–1.03 in the pushed brief, measured 1.0029 — and showed in-run that the same `matvec` streams 33.3 GB/s there, so the packed path was never memory-bound. **§12.5's "no outlier left to chase" is retracted.** Predicted 1.5–1.7×, got 1.349×: the next binder is the kernel again, at 22.5 GB/s. Previously: **`--bench` carries a CONTENTION WITNESS, gated three ways** (`probes/E7_REAL_LARGE_DONOR.md` §11, `WITNESS-CONFIRMED`): every rate this programme publishes from here on comes with an `ffn~` reading that says whether the machine was quiet, and **it was shown to FIRE on a known-positive** (+34.3% under load) before any of its nulls were allowed to count. It **failed its first gate** at 0.9877 against 0.990 — a 1.2% cost against a 0.008% prediction, 150× off — was rebuilt from 48 timestamps per token to 2, and passes at **0.9996** with `--logits` byte-identical. **It validates nothing retroactively**: every rate in `SPEED_LEDGER` §§12–19 predates it. Previously: **a defect, and it is general: `--bench` had NO CONTENTION WITNESS** — the `ffn`-invariance witness that makes E7 §8.2 and ledger §17 trustworthy exists only under `--profile`, so **every un-profiled rate this programme has published rests on the operator's belief that the machine was idle, with no instrument able to contradict it.** Found by two pre-registered attempts to read E4's `avx4` win as a rate on the real donor: the 300 cell was the **null called in advance**, the 1600 cell **could not decide** (baseline spread 6.1% against a 5.7% effect), and the interleaved re-run was **VOID by its own band** (paired median 0.973 against 1.00–1.09, within-arm dispersion 11.5%). The win *does* reach the wall — ~6–7% at 1600, established from profiled cells whose organs sum to the wall to **0.03%** and whose non-attention remainder is arm-invariant — but it is **not published as a rate**. Previously: **`SPEED_LEDGER` §19 splits the 16.2× gap to 50 tok/s for the first time — arithmetic on published numbers, no new data: at most **1.7–2.5×** of it is reachable by engine work, because a perfect engine (every weight organ at a streaming bandwidth this machine has been MEASURED delivering, and `f` already at E4's best) tops out at **5.14–7.61 tok/s** on a dense 10.6 B. **6.6–9.7× is not available at this shape by any engine work at all.** As a budget: 50 tok/s permits **4.6%** of a 10 B active per token today and **at most ~11%** with a perfect engine — so engine work is worth a ~2.5× larger sparsity budget and is not a substitute for the sparsity. Weakest assumption named in §19.5: that the weight organs are bandwidth-bound at all — §13.4 found attention was LATENCY-bound and E4 got 6.53× there, so if `P` (item 0) is the same the engineering share is larger. Previously: E7 extended twice: **`f` is LINEAR to 1600 tokens** (0.0319 ms per token of context, 0.2% agreement over a 5.3× range — E4's owed item 3 closed), the fixed-cost reading of the 300/800 inversion is **REFUTED** (it was 2.7%, inside ±5%), and `donor_engine.c:116` defaults the attention kernel to **`serial`**, not E4's `avx4` — a documentation defect that costs no ledger number, since E4 published the gap as +3.9%. Previously: E7 closed, `REAL-WEIGHTS-CONFIRMED`: a **real 7.62 B** donor (Qwen2.5-Coder-7B, 7.07 B active/token) is exported, loaded from a **30.46 GB** file, and reproduces PyTorch's greedy continuation **160/160**. It decodes at **4.460 tok/s** @300 — **11.2× short of 50** — against a band of 4.0–5.4 fixed before the export existed. And the synthetic proxy is finally checked: `T10` delivers 32.8 G-weights/s, the real donor **31.5**, −4.0% — inside the ±5% band, so E3/E4/E5's target-scale numbers stand.) Previously: (E6 closed, `GENERATION-CONFIRMED`: the engine grew a fourth mode, `--generate`, and a real fp32 Qwen2.5-0.5B reproduces PyTorch's greedy continuation **160 of 160 tokens**, five prompts, zero divergences — the first time any donor has chosen a token on this runtime. The ternary arms, the planted control, agree 1.9% and 6.2% and do not write language. And a correction: `1000/f` is a **ceiling**, not a rate — measured throughput at the target shape is **3.09–3.14 tok/s**, not 78.5.)** Previously: (E3 closed, `RESERVATION-BREAKS`: the engine is measured at the target shape for the first time — 3.090 tok/s on a 10.6 B, and the non-weight cost is 9–22× the reservation the budget was built on. §7's twelve-donor screen is superseded).**
