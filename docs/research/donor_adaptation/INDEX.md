...
**Last updated: 2026-09-08 (**E18 `CLIFF-NOT-SLOPE`: ternarizing ANY SINGLE ORGAN of a pretrained donor destroys its argmax, and 50 tok/s is a ~10%-activation budget** (`probes/E18_THE_RANKING_LADDER.md`, part B pre-registered `2ac74f5`). **First: the floor E17 said was the cheapest open item.** Computed from the reference continuations alone -- no engine, no weights -- the best possible CONSTANT predictor scores **11/160 at 0.5 B and 12/160 at 1.5 B, both `'\n'`**. E17's best ternary arm scored **exactly 12/160**: **every ternary arm this programme has ever built is at or BELOW a model that emits one token forever.** Then the ladder, seven arms, five never generated with before: `base` and the identity arm at **160/160**, and then **`H` 9, `A` 4, `F` 5, `FA` 12, `FAH` 10 -- ALL AT THE FLOOR.** The sharpest cell is `H`: it ternarizes **one tensor**, leaves the entire body fp32, costs **+0.338989 BPB** and sits **2.963 BELOW the chance line** -- a better BPB than most arms this programme ships -- and it agrees with its own donor on **9 of 160** tokens. **There is no rung between exact and broken.** The harness earned that reading first: **`G-L2` reproduces the engine EXACTLY, 12 and 10 on the nose**, before one empty rung was read. Across the 5 converted arms **r(BPB, agreement) = +0.4989** -- the WRONG sign, over a 2.377667 BPB span -- so the decoupling is a property of CONVERTED models, not of bad ones. **And the speed axis of the same ladder closes the other half**: derived over measured quantities only (bytes/weight RECOMPUTED from the artifact = exactly 0.500000), **50 tok/s permits 0.98-1.06 G active ternary weights = 9.8-10.6% of a 10 B model**, while **the FASTEST rung is the one that converts EVERYTHING and it is 6.9x short** -- quality improves down that table and speed improves up it. **No speed number moves: 6.79 tok/s stays EXACT.** **Three called directions, three misses** -- what saved the reading was brief section 6 registering the alternative outcome BEFORE the data existed. **REGOLA NUOVA: registrare l'esito ALTERNATIVO vale piu che azzeccare la direzione, perche e l'alternativa a proteggere la lettura quando la direzione sbaglia.** **SCOPE: E18 prova la conversione SENZA healing/QAT -- l'unico ramo rimasto in piedi.**)**
