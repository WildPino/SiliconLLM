...
### S2 — Entropy-coded weights — **MEASURED NEGATIVE on the from-scratch pool (2026-07-17, $0; see INVENTORE_01)**
Measured on `moe_gran.pt` (engine-exact trits): the pool is nearly maximum-entropy — H0 = 1.5835 vs log2(3) = 1.585, zeros only ~31%, no pair/Markov structure (Hpair/2 = H1 = 1.583). Entropy coding buys ~0 beyond the flat 5-trits/byte pack; the real 2.5× vs today is the already-queued trit-pack itself. Idea survives only as a conditional: re-check on QAT'd ladder checkpoints (different training pressure may sparsify weights). Original desk text kept below for the record.

*(original desk hypothesis)* The compressor compresses the model (fusing the two eras)
...
### S3 — Delta-coded expert pool — **MEASURED NEGATIVE on the from-scratch pool (2026-07-17, $0; see INVENTORE_01)**
Measured on `moe_gran.pt`: aligned mode-centroid agreement ≈ shuffled control (43% vs 42%); pool-wide nearest-row match ≈ control (delta-coding LOSES to direct coding on every matrix, net saving 0.0%). The from-scratch pool has no exploitable near-duplicate structure. Two live remnants: (a) fp32 rows DO correlate geometrically (|cos| NN ~0.45-0.5 vs ~0.26 random) — structure exists but ternarization does not expose it as duplicates → this fed the S1 probe; (b) the ladder's upcycled checkpoints are born with 4× replica seeding → re-run on MVE/S0 before any verdict there. Original desk text kept below for the record.

*(original desk hypothesis)* Delta-coded expert pool (the i.i.d. license)
...
### S4 — The silicon-native tokenizer — **FEASIBILITY MEASURED POSITIVE 2026-07-17 (input side; see INVENTORE_01)**
Measured on the pinned code-val: BPE-1024 spends forward passes with CV 0.75 across tokens (16.9% of tokens carry <2 bits; one token peaks at 87 bits); constant-entropy patches at matched threshold give CV 0.33 AND 31% fewer units, dialing to 3× shorter sequences at B=16 bits. Input-side thesis quantified; the open half is the output side (emitting variable-byte patches — BLT local-decoder problem) → stays v2, now with numbers. Original desk text below.

*(original desk hypothesis)* constant-entropy patches (K4, home-grown)
