...
**Status: FROZEN SPEC v1 for the S-class ladder — 2026-07-12 (Phase 64 design gate: decisions `PHASE64_DECISIONS.md`, sizing `PHASE64_BUDGET.md`, training `PHASE64_TRAINING_PLAN.md`; frozen configuration in §9). Originally a design draft (2026-06-30); the narrative sections below are kept as the rationale record.** Consolidates the validated foundation into one buildable architecture, sizes it against the measured Zen2 budget, and marks every open unknown with the probe that resolves it. This is the blueprint the C inference engine implements. Locked decisions are measured on the real hardware; per the Portability law all hardware numbers are runtime/config parameters — the 3600X (Zen2, AVX2/FMA, no VNNI, DDR4, L3 16MB/CCX ×2) is the *reference instance*, not the target. Narrative history: `HANDOFF.md`. Research detail: `docs/research/`.
...
**Second wall (measured, Phase 63): the per-position compute floor.** The keystone bounds *bytes*; it does not bound the elementwise per-position compute (scan recurrence, activations) — at the 8.3M config this floor is ~1.1-1.3 ms/token and *is* the current tok/s; ternary does not shrink it, and it grows per-component (proj-GEMV dominant, §8/64.0). The Phase 64 design gate must budget both walls together (it is this floor that scoped out the block-verify speedup at 8.3M, §3.8).

**Parametric restatement (64.0/64.1b measured — the frozen form):** the active per-token slice must fit the **aggregate LLC at the operating thread count** — threaded row-partition sees the sum of the CCX L3s (reference: 2×16 MB at t6, ~185 GB/s resident, declining slope to ~34-36 GB/s fully streamed; the single-CCX 16 MB figure remains the single-thread floor story). The wall is a measured *slope*, priced point-by-point by the `PHASE64_BUDGET.md` §1b rate curve, not a cliff.
...
**64.0 DONE (registered run, HEAD be5f448, 2026-07-06):** DRAM cold-stream single 21-26 GB/s / aggregate ceiling ≈ 40-44 GB/s saturated at 3 threads (the "~1.4-1.5×" derived input is closed: measured ~1.6-1.8×); compute-floor decomposition dense/MoE × t{1,6} — proj-GEMV dominates (51.6% ≈ P61's 52.7%, reproduced), MoE LUT-MLP is thread-flat (×1.06, gather/kernel-bound at ~4.2 GB/s effective — NOT at the bandwidth ceiling); proj-GEMV at t6 runs at aggregate BW (~40 GB/s) → L3 spill is a slope, not a cliff. **64.1 DONE (2026-07-06): `PHASE64_BUDGET.md` — verdict: CONSTRAINT INVERSION.** In the trainable region (P3: ~100-500M params) speed is slack by 12-130× over the floor and footprint by ~20-300×; the binding constraints are the training budget, licensed data at scale, and quality-per-param. 64.2 therefore optimizes quality under P3, not tok/s under P1; postures per decision recorded in the budget doc §5 (block-verify OUT for v1; recall tier PROMOTED to load-bearing; Finding-7 DEFERRED; vocab 2048-4096; MoE = the capacity lever). **64.1b DONE (registered microbenches, HEAD f4a53cf, 2026-07-12): both open CPU unknowns closed and folded into `PHASE64_BUDGET.md` §1b — (a) proj-GEMV r(size) curve confirms the aggregate-L3 row-partition hypothesis (t6 ~185 GB/s resident, declining slope to ~34-36 GB/s streamed; spill = slope, measured point by point); (b) expert-pool kernel-pure rate 7.45→17.0 GB/s (t1→t6, ×2.29) vs engine-integrated 4.2 → the gap is ~8.4 µs/expert of dispatch overhead around the kernel, an engineering lever, not a bandwidth wall. Verdict unchanged, strengthened: the candidate grid is a conservative floor.** **64.2 DONE (2026-07-12): `PHASE64_DECISIONS.md` — the decision set D1-D9 adjudicated (recipe invariance = the ladder contract; MoE = E-dial at fixed active, granularity quality-sealed; vocab V∈{2048,4096} decided by a rung-1 BPB A/B under the KD-cross-tokenizer constraint, boundary-coincident span-mapping KD with sequence-level fallback gated at the MVE; recall tier IN for v1 conditional-sealed with a pre-registered demotion path; block-verify OUT; K2 OUT of the ladder; Finding-7 deferred; threads IN; fp32 organs sealed). The ladder is bound: MVE 22M → S0 ~30M → S1 105M → S2 206M, one recipe, per-rung property gates + export gate at the top. One 64.3 probe engaged: the recall C-side two-stage query smoke (≤50 µs/token @128K entries, parity-neutral — sealed before its numbers).** **64.3 DONE (registered run, HEAD 7672107, 2026-07-12): PASS — 29.05 µs/token @128K at the operating point (t6; gate interpretation sealed pre-run on the budget's declared t6 assumption), margin 1.72×, zero perturbation (standalone, golden bit-identical) → recall stays IN v1 (D4 clause 1 cleared; clause 2 — InfoNCE curriculum stability — stays armed at the MVE/rung-1). Records: t1 = 52.4 µs (scalar ADC, SIMD headroom to ~18 µs = engine-v2 item); footprint 1.69 MB searchable + 64 MB values in RAM latency-class. Design decisions are complete → 64.4.** **64.4 DELIVERED (2026-07-12): this document upgraded to FROZEN SPEC v1 (§9) + `PHASE64_TRAINING_PLAN.md` (the ladder costed on the owner's envelope, desk-brackets re-priced once at the MVE; scenarios $0 / ~€60 / ~€100) → STOP-A: the owner approves the plan and launches stage-0 ($0: pre-flights, data brief, DDP smoke, MVE); STOP-B (spend go/no-go) follows the MVE re-price.**

---

## 9. Frozen v1 configuration (Phase 64.2/64.3 — the spec the ladder trains and the engine serves)

- **Recipe (D1, invariant across all rungs):** Mamba-1 selective scan (dt_rank = D/16, expand 2, conv4) + **one** SWA layer (window 128, H = D/32, last block of each 6-layer group) + gated-dReLU **ternary** MoE-MLP; projections/SSM/head **fp32** (P61); decode hygiene locked (greedy, rep 1.2, win 128, no top-p); per-domain code BPE.
- **The ladder:** MVE 22M (TS, pipeline pilot) → S0 D256 N96 L8 E32×h128 top-8 ≈ 30M → S1 E128 = 105M → S2 E256 = **206M (the v1 product candidate)**. Active/token fixed at ~3.1 MB ternary + resident fp32 ~16.8 MB (inside the aggregate LLC at t6). Per-rung property gates + the export gate: `PHASE64_DECISIONS.md` §10.
- **Bounded open slots (sealed criteria, not open questions):** vocab V ∈ {2048, 4096} — rung-1 BPB A/B (D3); router aux Switch (validated) vs aux-free — declared training A/B (D2); KD = boundary-coincident span mapping with sequence-level fallback — MVE-gated (D3).
- **IN:** threads (P63); recall tier — one slot, state-query dim 128, IVF-Hadamard + 4-bit ADC shortlist ~64 → exact top-16, values RAM latency-class; 64.3 measured 29 µs/token @128K at t6, gate PASS; demotion clause 2 (curriculum stability) stays armed at the MVE (D4).
- **OUT of v1:** block-verify (chassis banked behind `--block`, P63 law), K2 depth-reuse (unvalidated — would confound the ladder trend), Finding-7 SKIP (deferred, parked not abandoned), probe-5 compressed head (V ≤ 4K keeps the head hot), M-class+ (P3-bound; M1 = stretch row only).
