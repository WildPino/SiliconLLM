...
| **the Media Manager** | Owns repository hygiene, commits, branches and pushes. Creates and maintains the isolated research branch `codex/research/donor-adaptation`, records its base SHA, and never touches the concurrently running training branch. Is the only agent allowed to commit or push. Does not alter scientific conclusions or source code logic except repository metadata explicitly assigned to it. |
