...
Last updated: 2026-07-03 — **The weight-streaming half of the thesis is validated and the deliverable exists: a parity-gated C inference engine runs the whole architecture at 176 → 848 tok/s on the Zen 2 target (Phase 60, E1–E4). Phase 61 closed: SSM-projection ternarization rejected (precision-hungry control organs) — the fp32 projections stay; the memory-bound GEMV floor routes speed to E5 (threads + block-decode), not kernel tuning. Next: R1 seed-variance calibration, then the Cat-A product thesis (Phase 62).** Public releases: v0.1.0 (foundation), v0.2.0 (engine); AGPL-3.0; Zenodo DOI 10.5281/zenodo.21128459.

> **Standing honesty caveats (apply to every training number below):** all A/B verdicts are **single-seed** (seed 0); all properties are measured on **TinyStories at ~8–22M params**; the declared product domain (Cat-A: code/logs/structured) is **not yet tested**; the recall tier (phase-56 line) and the engine core (phase-58/59 line) are **two separate lineages**, not yet fused; "1.58-bit ternary" is **packed at 4 bits/weight** in the current engine.
...
- **Probe-1 — ternary (FINDING-1, validated).** A `pshufb` byte-LUT ternary kernel runs **4.2–5.0×** faster than fp32 matvec on Zen 2, **bit-exact** vs a scalar integer reference, and the "fewer bits → faster" signature is monotone (the anti-int8 fingerprint; int8 dequant *stalls* without VNNI). Quality: a ternary MLP trained from scratch (QAT) costs **+0.028 BPB** at ~8.3M on TinyStories — an *upper bound* (the ternary run was still descending; single-seed; MLP-only, backbone kept fp32). int8-VNNI is officially replaced by the LUT path on our silicon.
- **Probe-2 — activation sparsity (FINDING-2, validated).** A gated **dReLU** MLP reaches **92% hidden / 79% gate** sparsity at **+0.0006 BPB** (matched 4k-step A/B — quality is ~free). Predictor-free skippable = **2.12×** shrink. Composes with ternary along an independent axis → **~17×** fewer MLP bytes/token vs fp32-dense (4 B ÷ 0.5 B = 8× × 2.12×; corrected 2026-07-30 from a mixed-basis "21×") (a composition of separately-measured deltas; matched-convergence A/B queued).
...
## Phase 61 — SSM-projection ternarization (2026-07-03, CLOSED)

E3.5 named the fp32 SSM projections as the bottleneck (52.7% of engine time). Pre-registered A/B: can they go ternary like the MLP?

- **Rejected — clean double-fail.** Arm A (all four projections ternary) **0.8940 BPB**, arm B (in/out only) **0.8972**, gate ≤ 0.8899 (fresh base 0.8757 + 0.010) — fail by +0.018 / +0.022. The selective-SSM projections are **precision-hungry control organs** — now a measured number, not a caution. Probe-1's mixed-precision recipe (ternary the MLP, keep backbone + projections fp32) exits this phase *measured*, not assumed. A ≈ B within bf16 noise → the cost does not depend on the split; no 12k rerun (curves diverge past step 2000, prior against).
- **Free-variance datum (worth more than the fail):** the phase-58 base recipe, refit under the phase-61 script, gives 0.8757 vs the 0.8799 anchor — same recipe, different script, **~0.004 BPB apart**. So **any single-seed delta below ~0.005 is not resolvable** (probe-2's dReLU +0.0006 = "zero within noise," as already labeled); valid comparisons stay within-script matched; absolute numbers are the canonical table. This makes the 3-seed calibration (R1) a genuine project need, not an external ask.
- **GEMV lever rejected by measurement (Architect self-correction).** The 2–4× vectorization headroom hypothesized at E3.5 assumed those GEMVs were FMA-latency-bound; the Builder measured them **memory-bound** — a 2.6× microbench turns into **−4% in-engine** (probe-3's ρ-law, at home). Sister-law to E4: a compute-bound microbench speedup does not compose to a memory-bound system. The floor is the *single-thread token-by-token* floor (the engine's pre-registered scope); the two levers against it are **threads** (aggregate L3 bandwidth) and **block-decode** (amortize weight traffic over N positions) = **E5** — not kernel tuning. The floor strengthens the case for the architectural change already queued.
- **Relation to the SSM-quantization literature.** Published post-training quantization work consistently finds the selective-SSM core fragile (Quamba, Quamba2), with dt_proj notably insensitive (Mamba-PTQ: "almost no outliers") and outliers concentrated in gate/output projections (MambaQuant) — consistent with our double-arm result, where keeping x/dt in fp32 did not rescue ternarized in/out projections. One divergence to declare: KL-Lens (CVPR 2026W) isolates x_proj as the dominant sensitive subtype, on a hybrid SSM-Transformer-MoE under PTQ; our measurement is QAT ternarization on pure Mamba-1 at ~8.3M in byte-level BPB — PTQ sensitivity and QAT cost are different quantities. To our knowledge no published work ternarizes SSM projections: PTQ at W4 on these layers collapses entirely (Mamba-PTQ), while our QAT measurement bounds the cost at +0.018–0.022 BPB.
