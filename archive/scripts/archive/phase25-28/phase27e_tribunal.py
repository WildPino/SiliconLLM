"""
Phase 27E: Gated Expert Eligibility Test
Tests TOK_PREV with dynamic MoE eligibility vs baseline.

Configurations tested:
  LZ6+TOKPFX             -- baseline
  LZ6+TOKPFX+TOKPREV     -- candidate (standard update)
  LZ6+TOKPFX+TOKPREV_ELIG-- candidate (gated update)
"""
...
def main():
    ...
    p("=" * 80)
    p("PHASE 27E: Gated Expert Eligibility Test")
    p("=" * 80)
    ...
    CONFIGS = [
        ...
        ("LZ6+TOKPFX+TOKPREV_ELIG",["--tok-prefix", "--tok-prev-elig"]),
    ]
    ...
    for ds_name, ds_path in DATASETS:
        ...
        p()
        p(f"  {'Config':<25} {'BPB(quant)':>10}  {'dBPB':>8}  {'W_TOKPFX':>9}  {'W_TOKPREV':>9}")
        p("  " + "-" * 67)
        ...
        for cfg_label, _ in CONFIGS:
            ...
            p(f"  {cfg_label:<25} {fmt(m.get('quant_bpb')):>10}  {d:>8}  {w_tok:>9}  {w_prev:>9}")
    ...
    hdr = f"{'Config':<25}" + "".join(f"  {n[:12]:>12}" for n, _ in valid_ds)
    p(hdr)
    p("-" * len(hdr))
    for cfg_label, _ in CONFIGS:
        row = f"{cfg_label:<25}"
    ...
    report_path = os.path.join(RESULTS, "phase27e_tribunal_report.txt")
