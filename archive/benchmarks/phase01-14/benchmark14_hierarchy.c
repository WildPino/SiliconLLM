...
// -------------------------------------------------------------
// L1 Hierarchical Chunking (Delay-Line)
// -------------------------------------------------------------
float current_chunk_mean[64];
float chunk_history[32][128]; // Max 32 chunks, each 128D (64 Mean + 64 Last)
int chunk_count = 0;
...
void l1_reset() {
    ...
    memset(chunk_history, 0, sizeof(chunk_history));
    chunk_count = 0;
    total_l0_cyc = 0;
    total_l1_cyc = 0;
}

void extract_hierarchical(int global_idx, uint8_t byte, float* out, int C, int K) {
    ...
    if (C > 0 && K > 0) {
        ...
        for(int k_idx = 0; k_idx < K; k_idx++) {
            ...
            if (chunk_count - 1 - k_idx < 0) {
                memset(out + 64 + k_idx * 128, 0, 128 * sizeof(float)); // Zeri se non c'e' storia
            } else {
                memcpy(out + 64 + k_idx * 128, chunk_history[hist_idx], 128 * sizeof(float));
            }
        }
        ...
        if ((global_idx + 1) % C == 0) {
            float mean_last[128];
            for(int i=0; i<64; i++) {
                mean_last[i] = current_chunk_mean[i] / (float)C; // Mean
                mean_last[64 + i] = out[i];                     // Last
            }
            int dest = chunk_count % 32;
            memcpy(chunk_history[dest], mean_last, 128 * sizeof(float));
            chunk_count++;
            memset(current_chunk_mean, 0, sizeof(current_chunk_mean));
        }
    }
...
double run_eval(const char* name, int C, int K, int use_trigram, int print, double base_bpb) {
    l1_reset();
    l0_init(42);
    
    int num_features = (C > 0 && K > 0) ? (64 + K * 128) : (C == 0 ? 64 : 0);
    if (C == -1) num_features = 0; // Pure Baseline
    
    if (num_features > 0) {
        for (int i = 0; i < train_size; i++) extract_hierarchical(i, data[i+1], &features_train[i * num_features], C, K);
        l1_reset(); l0_init(42);
        for (int i = 0; i < val_size; i++) extract_hierarchical(i, data[train_size + i + 1], &features_val[i * num_features], C, K);
        l1_reset(); l0_init(42);
        for (int i = 0; i < test_size; i++) extract_hierarchical(i, data[train_size + val_size + i + 1], &features_test[i * num_features], C, K);
    ...
    if (print) {
        ...
        printf("| %-22s | %3d | %3d | %4d | %.4f | %7.1f | %7.1f | %7.1f | %9.5f |\n", 
            name, C, K, num_features, best_res_bpb, l0_c_p_b, l1_c_p_b, fwd_c_p_b, dbpb_1kc);
        fflush(stdout);
    }
    return best_res_bpb;
}
...
int main(int argc, char** argv) {
    ...
    printf("Starting Phase 14 Hierarchical Benchmark...\n");
    printf("Dataset: %s\n", filename);
    ...
    for(int use_trigram = 0; use_trigram <= 1; use_trigram++) {
        ...
        printf("| %-22s | %3s | %3s | %4s | %6s | %7s | %7s | %7s | %9s |\n", 
            "Model", "C", "K", "Dim", "BPB", "L0 Cyc", "L1 Cyc", "Fwd Cyc", "dBPB/1kC");
        printf("|------------------------|-----|-----|------|--------|---------|---------|---------|-----------|\n");
        ...
        int Cs[] = {4, 8, 16, 32};
        for(int i=0; i<4; i++) {
            char name[32]; sprintf(name, "L0 + L1 (C=%d)", Cs[i]);
            run_eval(name, Cs[i], K, use_trigram, 1, base_bpb);
        }
    }
