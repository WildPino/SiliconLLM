...
uint8_t ctx2_train[MAX_SAMPLES]; // For trigram
...
void evaluate_model_on_test(AdamState* model, int num_features, int use_residual, double* out_bpb, double* out_acc);
...
void _evaluate_model(AdamState* model, int num_features, int use_residual, double* out_bpb, double* out_acc, float* features, uint8_t* ctx, uint8_t* target, int size) {
...
void evaluate_model_on_val(AdamState* model, int num_features, int use_residual, double* out_bpb, double* out_acc) {
    _evaluate_model(model, num_features, use_residual, out_bpb, out_acc, features_val, ctx_val, target_val, val_size);
}

void evaluate_model_on_test(AdamState* model, int num_features, int use_residual, double* out_bpb, double* out_acc) {
    _evaluate_model(model, num_features, use_residual, out_bpb, out_acc, features_test, ctx_test, target_test, test_size);
}
...
void compute_ngrams(double alpha) {
    ...
    float* tri_counts = (float*)calloc(CLASSES*CLASSES*CLASSES, sizeof(float));
    float* tri_totals = (float*)calloc(CLASSES*CLASSES, sizeof(float));
    trigram_probs = (float*)malloc(CLASSES*CLASSES*CLASSES * sizeof(float));
    ...
    for (int i = 0; i < train_size; i++) {
        ...
        tri_counts[c2 * 65536 + c1 * 256 + t]++;
        tri_totals[c2 * 256 + c1]++;
    }
    ...
    for (int i = 0; i < CLASSES; i++) {
        ...
        for (int j = 0; j < CLASSES; j++) {
            ...
            bigram_logits[i][j] = (float)log2(bigram_probs[i][j]) * 0.6931471805599453f; // Convert log2 to natural log
            
            for (int k = 0; k < CLASSES; k++) {
                float prob = (tri_counts[i * 65536 + j * 256 + k] + alpha) / (tri_totals[i * 256 + j] + CLASSES * alpha);
                trigram_probs[i * 65536 + j * 256 + k] = prob;
            }
        }
    }
    free(tri_counts);
...
void eval_ngrams(double* uni_bpb, double* bi_bpb, double* tri_bpb) {
    double u_loss = 0;
    double b_loss = 0;
    double t_loss = 0;
...
int main(int argc, char** argv) {
    ...
    printf("Starting Phase 11 Generalization Benchmark...\n");
    printf("Dataset: %s\n", filename);
    fflush(stdout);
    ...
    fclose(f);
    printf("Data size loaded: %d bytes (Max 500,000)\n", data_size);
    fflush(stdout);
    ...
    for (int i = 0; i < train_size; i++) {
        ...
        ctx_train[i] = data[i];
        ctx2_train[i] = (i > 0) ? data[i-1] : 0;
    }
    
    // Fill Val
    for (int i = 0; i < val_size; i++) {
        int d_idx = train_size + i;
        target_val[i] = data[d_idx + 1];
        ctx_val[i] = data[d_idx];
        ctx2_val[i] = data[d_idx - 1];
    }
    
    // Fill Test
    for (int i = 0; i < test_size; i++) {
        int d_idx = train_size + val_size + i;
        target_test[i] = data[d_idx + 1];
        ctx_test[i] = data[d_idx];
        ctx2_test[i] = data[d_idx - 1];
    }
    ...
    compute_ngrams(0.1);
    double u_bpb, b_bpb, t_bpb;
    eval_ngrams(&u_bpb, &b_bpb, &t_bpb);
    printf("Unigram Test BPB: %.4f\n", u_bpb);
    printf("Bigram Test BPB:  %.4f\n", b_bpb);
    printf("Trigram Test BPB: %.4f\n\n", t_bpb);
    fflush(stdout);
