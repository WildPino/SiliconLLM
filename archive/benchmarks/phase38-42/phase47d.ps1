# Phase 47.D - Real Rollout Training, Not Proxy Corruption (H32)
#
# 47.C ha chiuso i proxy one-step: il training robusto vince su valC (non e' finto) ma non
# trasferisce al closed-loop -> il problema e' la DERIVA AUTOREGRESSIVA MULTI-STEP, non
# "un byte sporco ogni tanto". Diagnosi: "il decoder statico corregge perturbazioni locali
# ma non ha imparato il campo vettoriale degli stati che crea in autonomia."
# 47.D: rollout VERI - burst 8/16/32 byte campionati dal decoder STESSO (round iterativi),
# mix clean+rollout 80/20 (variante 70/30), CE sul byte vero + consistency leggera verso lo
# stato pulito, NO label smoothing. C3burst_h32 e' l'indizio (topBi quasi passa, selfBPB
# 0.67 collassa): serve rollout meno degenerativo, non piu' smoothing.
#
# Gate due stadi: MINI -> FULL solo se topBi scende vs A0 mlpH32 E selfBPB sano (esplicito).
#
# Run:  .\benchmarks\phase38-42\phase47d.ps1

$ROOT    = Split-Path (Split-Path $PSScriptRoot)
$BINDIR  = $ROOT + '\bin'
$WDIR    = $ROOT + '\weights'
$RDIR    = $ROOT + '\results\phase47d'
...
$WP      = $WDIR + '\phase47d'
$A0_H32  = $WDIR + '\phase47a0_mlpH32.bin'      # unregularized clean ref
$C3B_W   = $WDIR + '\phase47c_C3burst_h32.bin'  # 47.C clue (proxy burst), comparison ref
$OLD_A0  = $ROOT + '\results\phase47a0\phase47a0_gauntlet.txt'
$OLD_C   = $ROOT + '\results\phase47c\phase47c_train.txt'
$C2A_BPB = 2.2593
$D1_BPB  = 2.2522
foreach ($w in @($C2A_W,$D1_W,$A0_H32)) { if (-not (Test-Path $w)) { Write-Error "Missing weight: $w"; exit 1 } }
...
$THROTTLE = 12
$TEL_SEEDS = 2
$DCFGS = @('D8_h32','D16_h32','D32_h32','D16mix30_h32','D16nc_h32')
...
& gcc @GCC_FLAGS 'benchmarks/phase38-42/phase47d_rollout.c' @SRC_CORE -o "$BINDIR\phase47d_rollout.exe"
if ($LASTEXITCODE -ne 0) { Write-Error 'Compile failed: phase47d_rollout'; exit 1 }
...
$TRAINERS = @('phase44a_boundary','phase44b_homeostasis','phase44c_delta','phase44d_readout','phase44e_caps','phase44f_captrain','phase45a_relmove','phase45b_geometry','phase45c_gate','phase46a_l3','phase46b_l3','phase47a0_gauntlet','phase47b_decoder','phase47c_robust','phase47d_rollout')
...
# -------- Stage 1: rollout training (5 configs, iterative rounds) ----------------
Write-Host ''
Write-Host '=== Phase 47.D - real rollout training (5 config H32, 2 round DAgger-style) ===' -ForegroundColor Yellow
$TRAIN_OUT = $RDIR + '\phase47d_train.txt'
& "$BINDIR\phase47d_rollout.exe" $TS_DATA $D1_W $WP 2>&1 | Tee-Object $TRAIN_OUT
...
$CP    = if (Test-Path $OLD_C) { Parse-KV $OLD_C 'PROBE' } else { @{} }
if (-not $PROBE.ContainsKey('frozenD1') -or -not $PROBE.ContainsKey('D16nc_h32')) { Write-Error 'Train output incomplete'; exit 1 }
...
# -------- TF table + rollout collapse diagnostic ----------------------------------------
Write-Host ''
Write-Host '=== TF probes + rollout self-BPB per round (verso 1.5-2 = meno degenerativo; <1 = collasso) ===' -ForegroundColor Yellow
Write-Host ('  {0,-14} {1,8} {2,8} {3,8} {4,8} {5,7} {6,7} {7,8} {8,8}' -f 'probe','val1','val2','val3','valC','roll1','roll2','rms1','ent1')
foreach ($n in (@('frozenD1') + $DCFGS)) {
    if (-not $PROBE.ContainsKey($n)) { continue }
    $p = $PROBE[$n]
    $r1 = if ($p.ContainsKey('roll1')) { $p['roll1'] } else { '-' }
    $r2 = if ($p.ContainsKey('roll2')) { $p['roll2'] } else { '-' }
    Write-Host ('  {0,-14} {1,8} {2,8} {3,8} {4,8} {5,7} {6,7} {7,8} {8,8}' -f $n,$p['val1'],$p['val2'],$p['val3'],$p['valC'],$r1,$r2,$p['rms1'],$p['ent1'])
}
...
if ((Test-Path $C3B_W) -and $CP.ContainsKey('C3burst_h32')) {
    $gen_configs += @{ label='C3burst_h32'; exe='phase47_generator.exe'; wargs=('"'+$D1_W+'" "'+$C3B_W+'"'); wfile=$C3B_W; valbpb=(AvgVal $CP['C3burst_h32']); ref=$true }
}
foreach ($n in $DCFGS) {
...
Write-Host ''
Write-Host '======== MINI (criterio esplicito: topBi < mlpH32 ref E selfBPB sano a entrambi i temp) ========' -ForegroundColor Cyan
...
foreach ($cfg in $gen_configs) {
    ...
    $promo = '-'
    if (-not $cfg.ref) {
        $r65 = Worst $mini_agg 'mlpH32' '0.65'; $r55 = Worst $mini_agg 'mlpH32' '0.55'
...
if ($promoted.Count -eq 0) {
    Write-Host ''
    Write-Host '=== Verdetto 47.D (mini) ===' -ForegroundColor Yellow
    Write-Host '  -> Nessun rollout config abbassa topBi con selfBPB sano. Leggi roll1->roll2 nella' -ForegroundColor Yellow
    Write-Host '     tabella TF: se roll resta <1 il decoder collassa ancora nel proprio attrattore' -ForegroundColor Yellow
    Write-Host '     anche dopo aver visto la sua scia -> il campo vettoriale autonomo non e''' -ForegroundColor Yellow
    Write-Host '     apprendibile con K<=32 a queste dosi; valutare K maggiori, piu'' round, o' -ForegroundColor Yellow
    Write-Host '     ammettere che serve contesto phrase-scale (ma SOLO dopo questo dato).' -ForegroundColor Yellow
    Write-Host ''
...
# -------- Verdict -------------------------------------------------------------------
Write-Host ''
Write-Host '=== Verdetto 47.D ===' -ForegroundColor Yellow
$winners = @()
foreach ($mcfg in $promoted) {
...
if ($winners.Count -gt 0) {
    Write-Host ''
    Write-Host ('  -> LOOP CHIUSO: {0} passa ENTRAMBI i temp. Il decoder statico ha imparato il' -f ($winners -join ', ')) -ForegroundColor Green
    Write-Host '     campo vettoriale dei propri stati via short-rollout training - senza memoria' -ForegroundColor Green
    Write-Host '     volatile, senza cap. -> promozione formale 47 (candidato SEE-V4 readout).' -ForegroundColor Green
} else {
    Write-Host ''
    Write-Host '  -> Promosse al full ma nessuna chiude. Confronta col mini e con C3burst_h32 ref:' -ForegroundColor Yellow
    Write-Host '     se il rollout vero migliora il proxy ma non basta, la deriva e'' piu'' lunga di K=32' -ForegroundColor Yellow
    Write-Host '     -> prossima decisione utente: rollout lunghi/round multipli vs contesto phrase-scale.' -ForegroundColor Yellow
}
Write-Host ''
Write-Host ('  Files: ' + $RDIR)
