# Phase 47.C - Closed-Loop Robustness Training (exposure mismatch, NOT dose tuning)
#
# 47.B ha chiuso "e' solo overconfidence calibrabile": B2rms abbassa RMS ma topBi resta
# (la scala non e' la causa primaria); B1ls abbassa topBi distruggendo la lingua; B4drop
# sano ma insufficiente. Nuova diagnosi: EXPOSURE MISMATCH - il decoder statico e' bravo
# su feature da prefisso vero, in generazione incontra stati prodotti da se' e non rientra.
# 47.C: stesso decoder H32/H64, training su stati sporcati (byte-corrupt p02/p05 da
# D1-sample, scheduled bursts 12/256, consistency loss clean/corrupt, SEE-only vs SEE+D1).
# Target sempre il byte VERO. Nessuna L2/L3, nessun cap inferenza, nessun word counter.
#
# Gate a due stadi (utente): MINI word-gate (4 seed x 1 rng) su tutte le config ->
# promozione a FULL solo se topBi scende vs il ref non-robusto A0 alla stessa H senza
# selfBPB esplodere -> FULL word-gate (16 x 2) standard, promozione = ENTRAMBI i temp.
#
# Run:  .\benchmarks\phase38-42\phase47c.ps1

$ROOT    = Split-Path (Split-Path $PSScriptRoot)
$BINDIR  = $ROOT + '\bin'
$WDIR    = $ROOT + '\weights'
$RDIR    = $ROOT + '\results\phase47c'
...
$WP      = $WDIR + '\phase47c'
$A0_H32  = $WDIR + '\phase47a0_mlpH32.bin'
...
$THROTTLE = 12
$TEL_SEEDS = 2
$CCFGS = @('C1corr02_h32','C1corr02_h64','C2corr05_h32','C2corr05_h64','C3burst_h32','C3burst_h64','C4cons_h32','C4cons_h64','C0see_h64','C5see_h64')
...
& gcc @GCC_FLAGS 'benchmarks/phase38-42/phase47c_robust.c' @SRC_CORE -o "$BINDIR\phase47c_robust.exe"
if ($LASTEXITCODE -ne 0) { Write-Error 'Compile failed: phase47c_robust'; exit 1 }
...
$TRAINERS = @('phase44a_boundary','phase44b_homeostasis','phase44c_delta','phase44d_readout','phase44e_caps','phase44f_captrain','phase45a_relmove','phase45b_geometry','phase45c_gate','phase46a_l3','phase46b_l3','phase47a0_gauntlet','phase47b_decoder','phase47c_robust')
...
# -------- Stage 1: train the matrix (8 windows x 1M, ~8 GB RAM) -------------------
Write-Host ''
Write-Host '=== Phase 47.C - robustness matrix (10 configs, exposure-varied training) ===' -ForegroundColor Yellow
$TRAIN_OUT = $RDIR + '\phase47c_train.txt'
& "$BINDIR\phase47c_robust.exe" $TS_DATA $D1_W $WP 2>&1 | Tee-Object $TRAIN_OUT
...
$A0P   = Parse-KV $OLD_OUT  'PROBE'
if (-not $PROBE.ContainsKey('frozenD1') -or -not $PROBE.ContainsKey('C5see_h64')) { Write-Error 'Train output incomplete'; exit 1 }
...
# -------- TF table (val clean + valC robustness transfer) ------------------------------
Write-Host ''
Write-Host '=== TF probes (valC = transfer su stream sporcato p0.05; A0 refs: mlpH32 2.236avg, mlpH64 2.163avg) ===' -ForegroundColor Yellow
Write-Host ('  {0,-13} {1,8} {2,8} {3,8} {4,8} {5,8} {6,8} {7,8}' -f 'probe','val1','val2','val3','valC','rms1','ent1','maxp1')
foreach ($n in (@('frozenD1') + $CCFGS)) {
    if (-not $PROBE.ContainsKey($n)) { continue }
    $p = $PROBE[$n]
    Write-Host ('  {0,-13} {1,8} {2,8} {3,8} {4,8} {5,8} {6,8} {7,8}' -f $n,$p['val1'],$p['val2'],$p['val3'],$p['valC'],$p['rms1'],$p['ent1'],$p['maxp1'])
}
...
$gen_configs = @(
    @{ label='C2.A'; exe='phase43_generator.exe'; wargs=('"'+$C2A_W+'"'); wfile=$C2A_W; valbpb=$C2A_BPB; ref=$true; refH='' },
    @{ label='D1';   exe='phase44_generator.exe'; wargs=('"'+$D1_W+'"');  wfile=$D1_W;  valbpb=$D1_BPB;  ref=$true; refH='' },
    @{ label='mlpH32'; exe='phase47_generator.exe'; wargs=('"'+$D1_W+'" "'+$A0_H32+'"'); wfile=$A0_H32; valbpb=(AvgVal $A0P['mlpH32']); ref=$true; refH='' },
    @{ label='mlpH64'; exe='phase47_generator.exe'; wargs=('"'+$D1_W+'" "'+$A0_H64+'"'); wfile=$A0_H64; valbpb=(AvgVal $A0P['mlpH64']); ref=$true; refH='' }
)
foreach ($n in $CCFGS) {
    ...
    $rh = if ($n -like '*_h32') { 'mlpH32' } else { 'mlpH64' }
    $gen_configs += @{ label=$n; exe='phase47_generator.exe'; wargs=('"'+$D1_W+'" "'+$wf+'"'); wfile=$wf; valbpb=(AvgVal $PROBE[$n]); ref=$false; refH=$rh }
}
...
# -------- Stage 2: MINI word-gate (tutte le config) ---------------------------------------
$SEEDS_MINI = @()
...
Write-Host ''
Write-Host ('=== MINI word-gate: {0} runs ({1} seed x {2} rng, tutte le config) ===' -f $mini_tasks.Count,$NSEEDS_MINI,$RNGS_MINI.Count) -ForegroundColor Yellow
Invoke-Throttled $mini_tasks $THROTTLE
Aggregate $mini_tasks $mini_agg

Write-Host ''
Write-Host '======== MINI: worst-case per temp (criterio: topBi < ref stessa H, selfBPB sano) ========' -ForegroundColor Cyan
Write-Host ('  {0,-14} {1,7} {2,7} {3,7} {4,7} {5,8} {6,8} {7,7}' -f 'config','bi65','bi55','al55','ni55','self65','self55','promo')
$promoted = @()
foreach ($cfg in $gen_configs) {
    $w65 = Worst $mini_agg $cfg.label '0.65'; $w55 = Worst $mini_agg $cfg.label '0.55'
    $promo = '-'
    if (-not $cfg.ref) {
        $r65 = Worst $mini_agg $cfg.refH '0.65'; $r55 = Worst $mini_agg $cfg.refH '0.55'
    ...
    Write-Host ('  {0,-14} {1,7} {2,7} {3,7} {4,7} {5,8} {6,8} {7,7}' -f `
...
if ($promoted.Count -eq 0) {
    Write-Host ''
    Write-Host '=== Verdetto 47.C (mini) ===' -ForegroundColor Yellow
    Write-Host '  -> Nessuna config robusta abbassa topBi vs i ref non-robusti senza rompere selfBPB.' -ForegroundColor Yellow
    Write-Host '     L''exposure training (corruption/burst/consistency a queste dosi) non chiude il' -ForegroundColor Yellow
    Write-Host '     mismatch. Leggi valC nella tabella TF: se i robusti vincono su valC ma non in CL,' -ForegroundColor Yellow
    Write-Host '     la corruzione one-step non riproduce la deriva closed-loop (serve rollout vero).' -ForegroundColor Yellow
    Write-Host ''
    Write-Host ('  Files: ' + $RDIR)
    exit 0
}

# -------- Stage 3: FULL word-gate (solo promosse + refs) -----------------------------------
...
foreach ($tp in $TEMPS) {
    ...
    Write-Host ('  {0,-14} {1,9} {2,8} {3,7} {4,7} {5,7} {6,7} {7,8} {8,6}' -f 'config','valBPB','dC2.A','nameWst','runWst','topBi','altLp','selfBPB','gate')
    Write-Host ('  ' + '-'*86)
...
Write-Host ('  {0,-14} {1,9} {2,9} {3,9} {4,9} {5,9}   TF(rms/ent)' -f 'config','rms_p50','rms_p90','ent_p50','ent_p10','maxp_p90')
...
foreach ($mcfg in $promoted) {
    ...
    $p = $PROBE[$mcfg.label]
    Write-Host ('  {0,-14} {1,9} {2,9} {3,9} {4,9} {5,9}   {6}/{7}' -f `
        $mcfg.label,(Pctl $rms 0.5),(Pctl $rms 0.9),(Pctl $ent 0.5),(Pctl $ent 0.1),(Pctl $mxp 0.9),$p['rms1'],$p['ent1'])
}

# -------- Verdict -------------------------------------------------------------------
Write-Host ''
Write-Host '=== Verdetto 47.C ===' -ForegroundColor Yellow
$winners = @()
foreach ($mcfg in $promoted) {
    $p65 = Passes $mcfg '0.65'; $p55 = Passes $mcfg '0.55'
    $col = if ($p65 -and $p55) {'Green'} elseif ($p65) {'DarkCyan'} else {'Gray'}
    Write-Host ('  {0,-14} T0.65={1}  T0.55={2}  valBPB={3:F4}' -f $mcfg.label,($(if($p65){'PASS'}else{'fail'})),($(if($p55){'PASS'}else{'fail'})),$mcfg.valbpb) -ForegroundColor $col
    if ($p65 -and $p55) { $winners += $mcfg.label }
}
if ($winners.Count -gt 0) {
    Write-Host ''
    Write-Host ('  -> LOOP CHIUSO: {0} passa ENTRAMBI i temp. Il decoder statico robusto' -f ($winners -join ', ')) -ForegroundColor Green
    Write-Host '     (exposure training, zero modifiche a inferenza) comprime sotto gate E genera' -ForegroundColor Green
    Write-Host '     stabile. -> promozione formale 47 (candidato SEE-V4 readout).' -ForegroundColor Green
} else {
    Write-Host ''
    Write-Host '  -> Promosse al full ma nessuna passa entrambi i temp: l''exposure one-step aiuta' -ForegroundColor Yellow
    Write-Host '     ma non chiude. Confronta topBi full vs ref e SEE-only vs SEE+D1: decide se il' -ForegroundColor Yellow
    Write-Host '     prossimo passo e'' rollout piu'' lunghi/iterativi o l''asse SEE-substrate.' -ForegroundColor Yellow
}
Write-Host ''
Write-Host ('  Files: ' + $RDIR)
