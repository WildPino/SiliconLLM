...
def make_batch(B,T,P,Q,NK,NV,gen,device):
...
class RoutedAttn(nn.Module):    # Track A: learned coarse-IVF routing, refine EXACT (NO PQ). train=soft gate, eval=hard top-nprobe.
    # The recall slot routes each query to its top-nprobe codebook buckets and attends EXACTLY over the keys in
    # those buckets. The 3 arms differ ONLY in how the codebook C is learned -> isolates "does InfoNCE route better".
    def __init__(s,D,H,vatoms,nprobe,tau,learn):       # learn in {infonce, kmeans, random(frozen)}
        super().__init__(); s.H=H; s.hd=D//H; s.nprobe=nprobe; s.tau=tau; s.learn=learn; s.vatoms=vatoms
        s.qkv=nn.Linear(D,3*D,bias=False); s.o=nn.Linear(D,D,bias=False)
        s.C=nn.Parameter(torch.randn(vatoms,D)/math.sqrt(D),requires_grad=(learn!='random'))  # random=frozen LSH baseline
    ...
    def forward(s,x):
        ...
        att=(qh@(khq if s.refine=='pqadc' else kh).transpose(-1,-2))/math.sqrt(hd)  # pqadc: quantized keys; exact/rerank: exact keys
        ...
        if s.training:                                                     # SOFT differentiable gate (Gumbel-style routing)
            ...
            if s.refine=='rerank' and khq is not None:                    # 2-stage: cheap ADC shortlist -> EXACT rerank of top-kshort
                adc=(qh@khq.transpose(-1,-2)).sum(1)                      # (B,T,T) full-vector ADC score (sum over heads)
...
MAXPOS=8192
SIMVQ={'simvq':'infonce','simvq-kmeans':'kmeans','simvq-random':'random'}   # Track A arms -> codebook-learning mode
class Model(nn.Module):
    def __init__(s,vocab,D,L,arm,H,window,topk,N,vatoms=256,nprobe=4,rtau=1.0):
        super().__init__(); s.emb=nn.Embedding(vocab,D); s.pos=nn.Embedding(MAXPOS,D); slot=L-1
        mds={'swa':'window','sparse':'topk','attn':'full'}
        s.slot=slot; s.routed=None; layers=[]
        for i in range(L):
            if i==slot and arm in SIMVQ:
                rt=RoutedAttn(D,H,vatoms,nprobe,rtau,SIMVQ[arm]); s.routed=rt; layers.append(Layer(D,rt))
...
def eval_recall(model,a,NK,NV,dev,seed=777,quick=False):
    ...
    with torch.no_grad():
        for _ in range(nb):
            x,tgt,dist,_=make_batch(a.batch,a.ctx,a.pairs,a.queries,NK,NV,gen,dev)
...
def run_arm(arm,a,vocab,NK,NV,dev,deadline,resume_ck=None):
    ...
    model=Model(vocab,a.dim,a.layers,arm,a.heads,a.window,a.topk,a.nstate,a.vatoms,a.nprobe,a.route_tau).to(dev); model.use_ckpt=not a.no_ckpt
    ...
    elif a.init_from:                                                 # WARM-START: load WEIGHTS only (fresh opt/step) -> carry the grokked circuit to higher pairs
        ip=a.init_from if a.init_from.endswith('.pt') else os.path.join(a.init_from,arm,f"ckpt_{arm}.pt")
        if os.path.exists(ip):
            m=(model._orig_mod if hasattr(model,'_orig_mod') else model)
            m.load_state_dict(torch.load(ip,map_location=dev,weights_only=False)['model'])
            print(f"    [{arm:8s}] WARM-START weights from {ip} (fresh optimizer/step=0)",flush=True)
        else:
    ...
    def save_ck(step,finished,results=None):                          # atomic: write tmp then rename (no corrupt ckpt on kill)
        m=(model._orig_mod if hasattr(model,'_orig_mod') else model)
        ck={'arm':arm,'step':step,'done':finished,'best':best,'stale':stale,'npar':npar,'results':results,'cfg':vars(a),
    ...
    for step in range(start,a.steps):
        x,tgt,_,match=make_batch(a.batch,a.ctx,a.pairs,a.queries,NK,NV,gen,dev)
        opt.zero_grad()
        ...
        if step==start or (step+1)%ev==0:
            ov=eval_recall(model,a,NK,NV,dev,quick=True)[2]   # periodic overall recall: MQAR plateaus then JUMPS
    ...
    hit,tot,ov=eval_recall(model,a,NK,NV,dev)
...
def main():
    ...
    ap.add_argument("--arm",default="all",choices=["ssm","swa","sparse","attn","all","simvq","simvq-kmeans","simvq-random","simvq-all","ramp"])
    ...
    arms=(["ssm","swa","sparse","attn"] if a.arm=="all" else
          ...
          ["sparse","simvq","simvq-random"] if a.arm=="ramp" else [a.arm])   # ramp = MOBILE ceiling + bet + LSH control
    # --- recall-slot self-check: print the distinct mixer per arm; guard the swa==attn trap (win>=ctx = window mask is a no-op) ---
    rstr="RoutedAttn(V=%d,nprobe=%d,exact-refine)"%(a.vatoms,a.nprobe)
    mds={'ssm':'SSM','swa':"Attn(mode=window,win=%d)"%a.window,'sparse':"Attn(mode=topk,k=%d)"%a.topk,'attn':'Attn(mode=full)',
         'simvq':rstr+'/InfoNCE','simvq-kmeans':rstr+'/kmeans','simvq-random':rstr+'/random-frozen'}
    ...
    for arm in arms:
        ...
        if resume_ck is not None:
            if resume_ck.get('done') and resume_ck['step']>=a.steps and resume_ck.get('results') is not None:
