# Phase 46.B - LOWMARGIN at phrase scale (p30/p40) + PUNCT+LM, above D1
#
# 46.B0 found A3 LOWMARGIN fired ~0.01% in closed-loop because of the M=4 sustain (not a margin
# shift; p20 was just too low). 46.B raises the bar to p30/p40 (p40 ~ phrase scale) and adds a
# combined PUNCT+LOWMARGIN. Bd1 D1 control (no L3), B1 LOWMARGIN p30, B2 LOWMARGIN p40, B3 PUNCT+LM p30.
#
...
#
# Run:  .\benchmarks\phase38-42\phase46b_l3.ps1

$ROOT    = Split-Path (Split-Path $PSScriptRoot)
$BINDIR  = $ROOT + '\bin'
$WDIR    = $ROOT + '\weights'
$RDIR    = $ROOT + '\results\phase46b'
...
& gcc @GCC_FLAGS 'benchmarks/phase38-42/phase46b_l3.c' @SRC_CORE -o "$BINDIR\phase46b_l3.exe"
if ($LASTEXITCODE -ne 0) { Write-Error 'Compile failed: phase46b_l3'; exit 1 }
...
$TRAINERS = @('phase44a_boundary','phase44b_homeostasis','phase44c_delta','phase44d_readout','phase44e_caps','phase44f_captrain','phase45a_relmove','phase45b_geometry','phase45c_gate','phase46b_l3')
...
# -------- Train ---------------------------------------------------------------
Write-Host ''
Write-Host '=== Phase 46.B - LOWMARGIN p30/p40 + PUNCT+LM (Bd1..B3, fresh 320D extract per config) ===' -ForegroundColor Yellow
Write-Host '  320D features ~64 GB. Single trainer only. L3 schedule alters trajectory -> fresh per config.'
Write-Host ''
$TRAIN_OUT = $RDIR + '\phase46b_train.txt'
& "$BINDIR\phase46b_l3.exe" $TS_DATA $WP $C2A_W 2>&1 | Tee-Object $TRAIN_OUT
...
$sfxs = @('_Bd1_control','_B1_lm30','_B2_lm40','_B3_punctlm30')
$gen_configs = @(
...
foreach ($s in $sfxs) {
    $gen_configs += @{ label=$s.TrimStart('_'); exe='phase46_generator.exe'; weights=$WP+$s+'.bin'; valbpb=(Get-TrainBPB ($s+'.bin')); ref=$false; l3=($s -ne '_Bd1_control') }
}
$A0_BPB = Get-TrainBPB '_Bd1_control.bin'

# -------- Control reproduction check ------------------------------------------
Write-Host ''
Write-Host '=== Control reproduction (Bd1 = D1 with dead L3 block) ===' -ForegroundColor Yellow
$a0ok = ($null -ne $A0_BPB) -and ([math]::Abs($A0_BPB-$D1_BPB) -le 0.004)
Write-Host ('  Bd1 D1 control: BPB={0}  (expect ~{1})  {2}' -f ('{0:F4}' -f $A0_BPB),$D1_BPB,($(if($a0ok){'OK'}else{'CHECK'}))) -ForegroundColor $(if($a0ok){'Green'}else{'Yellow'})
...
foreach ($cfg in $gen_configs) {
    ...
    if ($both -and ($cfg.label -ne 'Bd1_control')) { $stable += $cfg.label }
}

Write-Host ''
Write-Host '=== Verdetto 46.B ===' -ForegroundColor Yellow
